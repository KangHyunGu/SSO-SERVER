...
import AllowSiteFormDialog from "src/components/admin/AllowSiteFormDialog.vue";
...
export default defineComponent({
  ...
  methods: {
    ...
    async removeItem(item){
      this.$q.dialog({
        label: '삭제',
        message: `${item.appName}을 삭제 하시겠습니까?`,
        cancel: true
      })
      .onOk(async () => {
        this.$q.loading.show();
        ...
        if(data) {
          ...
          this.$q.notify({type: 'info', message: `${item.appName}이 삭제되었습니다.`})
        }
      });
